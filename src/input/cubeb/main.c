...
EXP_FUNC void* wavaInput(void *audiodata) {
    ...
    wavaBailCondition(rv != CUBEB_OK, "Could not get preferred sample-rate: %s", cubeb_strerr(rv));
    ...
    wavaBailCondition(rv != CUBEB_OK, "Could not get minimum latency: %s", cubeb_strerr(rv));
    ...
    rv = cubeb_stream_init(app_ctx, &stm, "Audio capture process",
            ...
            data_cb, state_cb,
            &cubeb_data);
    wavaBailCondition(rv != CUBEB_OK, "Could not open the stream: %s", cubeb_strerr(rv));

    rv = cubeb_stream_start(stm);
    wavaBailCondition(rv != CUBEB_OK, "Could not start the stream: %s", cubeb_strerr(rv));
    ...
    wavaBailCondition(rv != CUBEB_OK, "Could not stop the stream: %s", cubeb_strerr(rv));
